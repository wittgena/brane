...
async def handle_call_tool(ctx: ServerRequestContext, params: types.CallToolRequestParams) -> types.CallToolResult:
    ...
    if params.name == "start-notification-stream":
        ...
        if items < 1 or items > 100:
            return types.CallToolResult(content=[types.TextContent(type="text", text="Error: items must be between 1 and 100")])
        if checkpoint_every < 1 or checkpoint_every > 20:
            return types.CallToolResult(content=[types.TextContent(type="text", text="Error: checkpoint_every must be between 1 and 20")])

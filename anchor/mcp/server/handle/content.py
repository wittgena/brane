# anchor.mcp.server.handle.content
...
async def handle_list_resources(ctx: ServerRequestContext, params: types.PaginatedRequestParams | None) -> types.ListResourcesResult:
    """정적 리소스와 페이지네이션이 적용된 아이템 목록을 함께 반환합니다."""
    ## 정적 리소스 (첫 페이지에만 표시)
    start = int(params.cursor) if params and params.cursor else 0
    resources = [
        types.Resource(uri=f"file:///{name}.txt", name=name, title=data["title"], mime_type="text/plain")
    ...
    return types.ListResourcesResult(resources=resources, next_cursor=next_cursor)

async def handle_read_resource(ctx: ServerRequestContext, params: types.ReadResourceRequestParams) -> types.ReadResourceResult:
    ...
    return types.ReadResourceResult(
        contents=[types.TextResourceContents(uri=str(params.uri), text=content, mime_type="text/plain")]
    )

async def handle_list_prompts(ctx: ServerRequestContext, params: types.PaginatedRequestParams | None) -> types.ListPromptsResult:
    return types.ListPromptsResult(
        prompts=[
            types.Prompt(
                ...
                arguments=[
                    types.PromptArgument(name="context", description="Additional context", required=False),
                    types.PromptArgument(name="topic", description="Specific topic", required=False),
                ],
            )
        ]
    )
...
async def handle_get_prompt(ctx: ServerRequestContext, params: types.GetPromptRequestParams) -> types.GetPromptResult:
    ...
    return types.GetPromptResult(messages=messages, description="Generated prompt messages")

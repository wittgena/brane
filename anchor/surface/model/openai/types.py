# anchor.surface.model.openai.types
## @lineage: anchor.surface.model.types.openai.types
## @lineage: anchor.surface.model.legacy.openai.types
...
from anchor.surface.model.llm.base import BaseOpenAIResponse
...
class OutputTokensDetails(BaseOpenAIResponse):
...
class InputTokensDetails(BaseOpenAIResponse):
...
class ResponseAPIUsage(BaseOpenAIResponse):
    input_tokens: int
...
class ResponsesAPIResponse(BaseOpenAIResponse):
    id: str
    created_at: int
...
class ReasoningSummaryTextDeltaEvent(BaseOpenAIResponse):
...
class ReasoningSummaryTextDoneEvent(BaseOpenAIResponse):
...
class ReasoningSummaryPartDoneEvent(BaseOpenAIResponse):
    type: Literal[ResponsesAPIStreamEvents.REASONING_SUMMARY_PART_DONE]
    item_id: str
    output_index: int
    sequence_number: int
    summary_index: int = 0
    part: BaseOpenAIResponse


class OutputItemAddedEvent(BaseOpenAIResponse):
    type: Literal[ResponsesAPIStreamEvents.OUTPUT_ITEM_ADDED]
    output_index: int
    item: Optional[BaseOpenAIResponse]


class OutputItemDoneEvent(BaseOpenAIResponse):
...
class ContentPartAddedEvent(BaseOpenAIResponse):
...
class ContentPartDonePartOutputText(BaseOpenAIResponse):
    type: Literal["output_text"]
    text: str
    annotations: List[BaseOpenAIResponse]
    logprobs: Optional[List[OpenAIChatCompletionLogprobsContent]]


class ContentPartDonePartRefusal(BaseOpenAIResponse):
    type: Literal["refusal"]
    refusal: str


class ContentPartDonePartReasoningText(BaseOpenAIResponse):
...
class ContentPartDoneEvent(BaseOpenAIResponse):
...
class OutputTextDeltaEvent(BaseOpenAIResponse):
...
class OutputTextAnnotationAddedEvent(BaseOpenAIResponse):
...
class OutputTextDoneEvent(BaseOpenAIResponse):
...
class RefusalDeltaEvent(BaseOpenAIResponse):
...
class RefusalDoneEvent(BaseOpenAIResponse):
...
class FunctionCallArgumentsDeltaEvent(BaseOpenAIResponse):
...
class FunctionCallArgumentsDoneEvent(BaseOpenAIResponse):
...
class MCPListToolsInProgressEvent(BaseOpenAIResponse):
...
class MCPListToolsCompletedEvent(BaseOpenAIResponse):
...
class MCPListToolsFailedEvent(BaseOpenAIResponse):
...
# MCP Call Events
class MCPCallInProgressEvent(BaseOpenAIResponse):
...
class MCPCallArgumentsDeltaEvent(BaseOpenAIResponse):
...
class MCPCallArgumentsDoneEvent(BaseOpenAIResponse):
...
class MCPCallCompletedEvent(BaseOpenAIResponse):
...
class MCPCallFailedEvent(BaseOpenAIResponse):
...
class ImageGenerationPartialImageEvent(BaseOpenAIResponse):
    type: Literal[ResponsesAPIStreamEvents.IMAGE_GENERATION_PARTIAL_IMAGE]
    partial_image_index: int
    b64_json: str


class ErrorEventError(BaseOpenAIResponse):
...
class ErrorEvent(BaseOpenAIResponse):
    type: Literal[ResponsesAPIStreamEvents.ERROR]
    sequence_number: int
    error: ErrorEventError


class GenericEvent(BaseOpenAIResponse):
    type: str
...
class OpenAIModerationResult(BaseOpenAIResponse):
...
class OpenAIModerationResponse(BaseOpenAIResponse):
    """

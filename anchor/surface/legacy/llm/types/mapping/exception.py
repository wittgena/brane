# anchor.surface.legacy.llm.types.mapping.exception
## @lineage: anchor.surface.legacy.types.mapping.exception
...
def extract_and_raise_litellm_exception(
    ...
    if match:
        ...
        raised_exception_obj = getattr(config, exception_name, None)
...
def exception_type(  # type: ignore  # noqa: PLR0915
    ...
    if any(
        ...
        for exc_type in config.LITELLM_EXCEPTION_TYPES
    ):
        return original_exception
    exception_mapping_worked = False
    exception_provider = custom_llm_provider
    if config.suppress_debug_info is False:
        print()  # noqa
        print(  # noqa
    ...
    try:
        ...
        if model:
            ...
            try:
                ...
                messages = get_first_chars_messages(kwargs=completion_kwargs)
                ...
                if (
                    messages
                    and len(messages) > 0
                    and config.redact_messages_in_exceptions is False
                ):
                    extra_information += f"\nMessages: `{messages}`"
            ...
            if (
                ...
                if hasattr(original_exception, "status_code"):
                    ...
                    if original_exception.status_code == 401:
                        ...
                        exception_mapping_worked = True
                        raise InternalServerError(
                            message=f"AnthropicException - {error_str}. Handle with `InternalServerError`.",
                            llm_provider="anthropic",
                            model=model,
                            response=getattr(original_exception, "response", None),
                        )
                        ...
                        exception_mapping_worked = True
                        raise ServiceUnavailableError(
                            message=f"AnthropicException - {error_str}. Handle with `ServiceUnavailableError`.",
                            llm_provider="anthropic",
                            model=model,
                            response=getattr(original_exception, "response", None),
                        )
            ...
            elif custom_llm_provider in config._openai_like_providers:
        ...
        if exception_mapping_worked:
            ...
            for error_type in config.LITELLM_EXCEPTION_TYPES:

...
from anchor.surface.model.openai.types import FileSearchTool, ResponsesAPIResponse, WebSearchOptions
from anchor.surface.model.types import ModelInfo, SearchContextCostPerQuery, StandardBuiltInToolsParams, PromptTokensDetailsWrapper
from anchor.channel.compat.switch.params import Message, ModelResponse, Usage
...
class BuiltInToolCostTracker:
    ...
    def get_cost_for_built_in_tools(
        ...
        # Handle web search
        if BuiltInToolCostTracker.response_object_includes_web_search_call(
            response_object=response_object, usage=usage
        ):
            return BuiltInToolCostTracker._handle_web_search_cost(
                model=model,
        ...
        # Handle file search
        if BuiltInToolCostTracker.response_object_includes_file_search_call(
            response_object=response_object
        ):
            return BuiltInToolCostTracker._handle_file_search_cost(
                model=model,
                custom_llm_provider=custom_llm_provider,
                standard_built_in_tools_params=standard_built_in_tools_params,
            )
        ...
        return BuiltInToolCostTracker._handle_azure_assistant_costs(
            model=model,
            custom_llm_provider=custom_llm_provider,
            standard_built_in_tools_params=standard_built_in_tools_params,
        )
    ...
    def _handle_web_search_cost(
        ...
        model_info = BuiltInToolCostTracker._safe_get_model_info(
        ...
        return BuiltInToolCostTracker.get_cost_for_web_search(
    ...
    def _handle_file_search_cost(
        ...
        storage_gb, days = BuiltInToolCostTracker._extract_file_search_params(
            file_search_usage
        )

        return BuiltInToolCostTracker.get_cost_for_file_search(
    ...
    def _handle_azure_assistant_costs(
        ...
        total_cost = 0.0
        total_cost += BuiltInToolCostTracker._get_vector_store_cost(
            model_info, custom_llm_provider, standard_built_in_tools_params
        )
        total_cost += BuiltInToolCostTracker._get_computer_use_cost(
            model_info, custom_llm_provider, standard_built_in_tools_params
        )
        total_cost += BuiltInToolCostTracker._get_code_interpreter_cost(
            model_info, custom_llm_provider, standard_built_in_tools_params
        )
    ...
    def _get_vector_store_cost(
        ...
        return BuiltInToolCostTracker.get_cost_for_vector_store(
    ...
    def _get_computer_use_cost(
        ...
        (
            input_tokens,
            output_tokens,
        ) = BuiltInToolCostTracker._extract_token_counts(computer_use_usage)

        return BuiltInToolCostTracker.get_cost_for_computer_use(
    ...
    def _get_code_interpreter_cost(
        ...
        model_info_dict = dict(model_info) if model_info is not None else None
        sessions = BuiltInToolCostTracker._safe_convert_to_int(
            code_interpreter_sessions
        )

        return BuiltInToolCostTracker.get_cost_for_code_interpreter(
            sessions=sessions,
            provider=custom_llm_provider,
            model_info=model_info_dict,
        )
    ...
    def _extract_token_counts(
        ...
        if isinstance(computer_use_usage, dict):
            ...
            input_tokens = BuiltInToolCostTracker._safe_convert_to_int(
                input_tokens_val
            )
            output_tokens = BuiltInToolCostTracker._safe_convert_to_int(
                output_tokens_val
            )
    ...
    def response_object_includes_web_search_call(
        ...
        if isinstance(response_object, ModelResponse):
            ...
            has_url_citations = (
                BuiltInToolCostTracker.response_includes_annotation_type(
            ...
            return BuiltInToolCostTracker.response_includes_output_type(
    ...
    def response_object_includes_file_search_call(
        ...
        if isinstance(response_object, ModelResponse):
            ...
            return BuiltInToolCostTracker.response_includes_annotation_type(
    ...
    def _safe_get_model_info(
        ...
        try:
            return config.get_model_info(model=model, custom_llm_provider=custom_llm_provider)
        except Exception:
            return None
    ...
    def get_cost_for_web_search(
        ...
        return BuiltInToolCostTracker.get_default_cost_for_web_search(
            model_info
        )
    ...
    def _get_code_interpreter_cost_from_model_map(
        ...
        try:
            ...
            if model_key and model_key in config.model_cost:
                return config.model_cost[model_key].get("code_interpreter_cost_per_session")
        except Exception:
            pass
    ...
    def get_cost_for_code_interpreter(
        ...
        if provider:
            cost_per_session = BuiltInToolCostTracker._get_code_interpreter_cost_from_model_map(
                provider=provider
            )
    ...
    def _get_web_search_options(kwargs: Dict) -> Optional[WebSearchOptions]:
        ...
        tools = BuiltInToolCostTracker._get_tools_from_kwargs(
            kwargs=kwargs, tool_type="web_search_preview"
        ) or BuiltInToolCostTracker._get_tools_from_kwargs(
        ...
        if tools:
            ...
            for tool in tools:
                if isinstance(tool, dict):
                    if BuiltInToolCostTracker._is_web_search_tool_call(tool):
    ...
    def _get_file_search_tool_call(kwargs: Dict) -> Optional[FileSearchTool]:
        ...
        if tools:
            for tool in tools:
                if isinstance(tool, dict):
                    if BuiltInToolCostTracker._is_file_search_tool_call(tool):

...
def _import_server(file: Path, server_object: str | None = None):  # pragma: no cover
    ...
    def _check_server_object(server_object: Any, object_name: str):
        ...
        if not isinstance(server_object, MCPServer):
            log.error(f"The server object {object_name} is of type {type(server_object)} (expecting {MCPServer}).")
            if isinstance(server_object, LowLevelServer):
                log.warning("Note that only MCPServer is supported. Low level Server class is not yet supported.")
            return False
        return True
        ...
        for name in ["mcp", "server", "app"]:
            if hasattr(module, name):
                if not _check_server_object(getattr(module, name), f"{file}:{name}"):
                    log.error(f"Ignoring object '{file}:{name}' as it's not a valid server object")
                    continue
...
def run(
    ...
    try:
        ...
        if final_transport in ["sse", "streamable-http"]:
            ## SDK의 구조에 따라 다르겠지만, 만약 starlette/fastapi 앱을 추출할 수 있다면:
            ## app = server.streamable_http_app(**kwargs)
            ## import uvicorn
            ## uvicorn_settings = script_config.get("uvicorn_kwargs", {})
            ## uvicorn.run(app, host="0.0.0.0", port=final_port, **uvicorn_settings)
            ## 또는 기본 server.run이 내부적으로 uvicorn을 처리한다면 값만 넘겨줍니다.
...
def install(
    ...
    server_name: Annotated[
        str | None,
        typer.Option("--name", "-n", help="Custom name for the server (defaults to server's name attribute or file name)"),
    ] = None,
    with_editable: Annotated[
        Path | None,
        typer.Option(
    ...
    if not claude.get_claude_config_path():
        log.error("Claude app not found")
        sys.exit(1)
        ...
        if env_file:
            if dotenv:
                try:
                    ...
                    log.exception("Failed to load .env file")
                    sys.exit(1)
            else:
                log.error("python-dotenv is not installed. Cannot load .env file.")
                sys.exit(1)
    ...
    if claude.update_claude_config(
        ...
        log.info(f"Successfully installed {name} in Claude app")
    else:
        log.error(f"Failed to install {name} in Claude app")
        sys.exit(1)

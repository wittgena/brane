# anchor.router.model.cost.track.toolcall
## @lineage: bound.router.model.cost.track.toolcall
## @lineage: bound.channel.model.cost.track.toolcall
## @lineage: channel.model.cost.track.toolcall
## @lineage: channel.cost.tool_call_cost_tracking
## @lineage: channel.gov.cost.tool_call_cost_tracking
## @lineage: channel.bound.cost.tool_call_cost_tracking
...
from anchor.config.constants import OPENAI_FILE_SEARCH_COST_PER_1K_CALLS
from anchor.router.model.types.llms.openai import FileSearchTool, ResponsesAPIResponse, WebSearchOptions
from anchor.router.model.types.utils import ModelInfo, SearchContextCostPerQuery, StandardBuiltInToolsParams, PromptTokensDetailsWrapper
...
class StandardBuiltInToolCostTracking:
    ...
    def get_cost_for_file_search(
        ...
        if provider == "azure":
            from anchor.config.constants import AZURE_FILE_SEARCH_COST_PER_GB_PER_DAY
    ...
    def get_cost_for_vector_store(
        ...
        if provider == "azure":
            from anchor.config.constants import AZURE_VECTOR_STORE_COST_PER_GB_PER_DAY

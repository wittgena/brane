# anchor.router.model.token.counter
...
from anchor.router.model.token.encoding import get_default_encoding
from bound.client.transport.http_handler import _get_httpx_client
from anchor.router.model.token.url_utils import safe_get
from anchor.router.model.types.llms.anthropic import (
...
from anchor.router.model.types.utils import SelectTokenizerResponse
...
def get_modified_max_tokens(
    ...
    try:
        ...
        log.debug(
            f"[token.counter] get_modified_max_tokens() - user_max_tokens: {user_max_tokens}"
        )

        return user_max_tokens
    except Exception as e:
        log.debug(
            "[token.counter.py] get_modified_max_tokens() - Error while checking max token limit: {}\nmodel={}, base_model={}".format(
...
class _MessageCountParams:
    ...
    def __init__(
        ...
        elif actual_model in config.open_ai_chat_completion_models:
            self.tokens_per_message = 3
            self.tokens_per_name = 1
        elif actual_model in config.azure_llms:
            self.tokens_per_message = 3
            self.tokens_per_name = 1
        else:
            self.tokens_per_message = 3
...
def token_counter(
    ...
    from anchor.router.model.token.convert import convert_list_message_to_dict
...
def _get_count_function(
    ...
    from anchor.router.model.token.tokenizer import _select_tokenizer

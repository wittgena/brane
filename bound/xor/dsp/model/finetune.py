...
from bound.xor.dsp.adapter import Adapter
from bound.xor.chat import ChatAdapter
from bound.xor.dsp.instance import LM
from bound.xor.dsp.handler.train import infer_data_format
from bound.xor.dsp.opt.predict import Predict
from bound.xor.dsp.model.prompter import Prompter
from bound.xor.exam.example import Example
from bound.xor.trace.bootstrap import bootstrap_trace_data
...
class BootstrapFinetune(FinetunePrompter):
    def __init__(
        self,
    ...
    def compile(
        ...
        log.info("Preparing the student and teacher programs...")
        ...
        log.info("Preparing the train data...")
        key_to_data = {}
        for pred_ind, pred in enumerate(student.predictors()):
            ...
            if training_key not in key_to_data:
                ...
                log.info(f"Using {len(train_data)} data points for fine-tuning the model: {pred.lm.model}")
                finetune_kwargs = {
                    "lm": pred.lm,
        ...
        log.info("Starting LM fine-tuning...")
        ...
        log.info(f"{len(key_to_data)} fine-tuning job(s) to start")
        key_to_lm = self.finetune_lms(key_to_data)

        log.info("Updating the student program with the fine-tuned LMs...")
        for pred_ind, pred in enumerate(student.predictors()):
            data_pred_ind = None if self.multitask else pred_ind
            training_key = (pred.lm, data_pred_ind)
        ...
        log.info("BootstrapFinetune has finished compiling the student program")
    ...
    def finetune_lms(finetune_dict) -> dict[Any, LM]:
        ...
        for ind, (key, job) in enumerate(key_to_job.items()):
            ...
            job.thread.join()
            log.info(f"Job {ind + 1}/{num_jobs} is done")
    ...
    def _prepare_finetune_data(self, trace_data: list[dict[str, Any]], lm: LM, pred_ind: int | None = None):
        ...
        if self.metric:
            ...
            log.info(f"After filtering with the metric, {len(trace_data)} examples remain")

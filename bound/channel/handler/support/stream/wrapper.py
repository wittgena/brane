...
from bound.channel.handler.support.get_api_base import get_api_base

from anchor.executor import executor
from anchor.exceptions import OpenAIError
from xphi.flow.rule.validator import Rules
from anchor.switch.params import ModelResponse, ModelResponseStream, StreamingChoices, Usage

from bound.channel.handler.support.stream.response.check import is_model_response_stream_empty
from bound.channel.handler.support.helpers import map_finish_reason, process_response_headers
from anchor.mapping.exception import exception_type
from bound.router.model.types.llms.openai import OpenAIChatCompletionChunk
from bound.router.model.types.provider import LlmProviders
from bound.router.model.types.router import GenericLiteLLMParams
from bound.router.model.types.utils import Delta, CallTypes, GenericStreamingChunk as GChunk
...
class CustomStreamWrapper:
    ...
    def return_processed_chunk_logic(  # noqa
        ...
        from bound.channel.handler.support.helpers import (
    ...
    def chunk_creator(self, chunk: Any):  # type: ignore  # noqa: PLR0915
        ...
        try:
            ...
            from bound.router.model.types.utils import GenericStreamingChunk as GChunk

            if (
                isinstance(chunk, ModelResponseStream)
                and self.custom_llm_provider is not None
                and self.custom_llm_provider in config._custom_providers
            ):
                _has_content = bool(
                    chunk.choices
            ...
            if (
                ...
                if anthropic_response_obj["usage"] is not None:
                    setattr(
                        model_response,
                        "usage",
                        config.Usage(**anthropic_response_obj["usage"]),
                    )
                ...
                if response_obj["usage"] is not None:
                    if isinstance(response_obj["usage"], dict):
                        ...
                        setattr(
                            model_response,
                            "usage",
                            config.Usage(**response_obj["usage"].model_dump()),
                        )
    ...
    async def fetch_stream(self):
        if self.completion_stream is None and self.make_call is not None:
            ...
            self.completion_stream = await self.make_call(
                client=config.module_level_aclient
            )
    ...
    def _handle_stream_fallback_error(self, e: Exception) -> "NoReturn":
        ...
        """
        from anchor.exceptions import MidStreamFallbackError
...
def convert_generic_chunk_to_model_response_stream(
    ...
    from bound.router.model.types.utils import Delta

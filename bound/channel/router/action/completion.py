# bound.channel.router.action.completion
## @lineage: anchor.router.action.completion
import asyncio
import contextvars
import datetime
import inspect
...
from typing import TYPE_CHECKING, Any, Dict, Iterable, List, Literal, Optional, Tuple, Type, Union, cast
if TYPE_CHECKING:
...
from bound.channel.router.action.param.optional import get_optional_params
...
from bound.channel.model.types.llms.openai import AllMessageValues
from bound.channel.secret.manager import get_secret_bool, get_secret_str
from anchor.mapping.exception import exception_type
from bound.handler.param.litellm import get_litellm_params
from bound.channel.model.info.support import supports_httpx_timeout
from anchor.exceptions import LiteLLMUnknownProvider
from bound.channel.router.action.timeout import CompletionTimeout
...
from bound.channel.router.action.param.validator import (
...
from bound.channel.bridge.llms.openai.completion import OpenAIChatCompletion
from bound.channel.model.types.llms.anthropic import AnthropicThinkingParam
...
from bound.channel.model.types.utils import all_litellm_params
...
def completion(  # type: ignore # noqa: PLR0915
    ...
    **kwargs,
    ...
    # 2. Provider 추론 및 Model Name 정규화
    ...
    # 3. 데이터 클래스 껍데기 세팅 (의존성 유지를 위한 최소 단위)
    ...
    # 4. 프롬프트 및 시스템 메시지 매핑 로직 (Surgent 환경 호환성)
    base_model = kwargs.get("base_model") or (kwargs.get("model_info", {}).get("base_model"))
    provider_config: Optional[Any] = None
    if custom_llm_provider in [p.value for p in LlmProviders]:
    ...
    # 5. 파라미터 필터링 (불필요한 인자를 각 Provider에 맞게 쳐냄)

# bound.channel.action.search.file
## @lineage: bound.bridge.action.search.file
## @lineage: bound.client.action.search.file
## @lineage: anchor.router.action.search.file
import json
import time
from typing import Any, Dict, Iterable, List, Optional, Tuple, Union, cast
from anchor.surface.legacy.api.aresponse import aresponses
from anchor.model.types.llms.openai import ResponseOutputItem, ResponsesAPIResponse
from anchor.model.types.vector_stores import VectorStoreSearchResult
...
ToolParam = Any
FILE_SEARCH_FUNCTION_NAME = "internal_file_search"
...
def _replace_file_search_tools(
    ...
    Returns: (new_tools_list, all_vector_store_ids)
    """
...
def _build_search_results_for_include(
    ...
    """Convert VectorStoreSearchResult objects to the format expected in file_search_call."""
...
def _synthesize_responses_api_response(
    ...
    synthesized = ResponsesAPIResponse(
        # [ToposId 반영] uuid4() 사용
        id=getattr(original_response, "id", f"resp_{uuid4().hex}"),
        object="response",
...
async def _call_aresponses(input, model, tools, **kwargs):
...
async def aresponses_with_emulated_file_search(
    ...
    **kwargs: Any,
    ...
    ## Replace file_search tools with function tool
    transformed_tools, all_vs_ids = _replace_file_search_tools(tools)

    ## First provider call
    with flow_scope(is_internal_call=True):
    ...
    ## Look for a file_search function_call in the output
    file_search_calls = [
        item
    ...
    ## Execute each file_search tool call
    file_search_call_id = f"fs_{uuid4().hex[:24]}"
    ...
    ## Synthesize OpenAI-format output

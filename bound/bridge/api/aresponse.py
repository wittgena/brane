# bound.bridge.api.aresponse
## @lineage: anchor.action.api.aresponse
import asyncio
import contextvars
...
from bound.bridge.api.handler import ResponseApiHandler
from bound.channel.handler.ws import ResponseWebsocketHandler
from bound.bridge.client.wrapper import client
...
from anchor.surface.legacy.llm.types.response import *
...
from anchor.surface.legacy.llm.openai.types import ResponseText
...
from bound.channel.support.param.litellm import get_litellm_params, infer_openai_data_residency
...
async def aresponses_api_with_mcp(
    ...
    **kwargs,
    ...
    (
        ...
        other_tools,
    ) = MCPPayloadUtils._parse_mcp_tools(tools)
    ...
    if secret_fields and isinstance(secret_fields, dict):
        (
            ...
            _,
            _,
        ) = ResponseIdentityManager.extract_mcp_headers_from_request(secret_fields=secret_fields, tools=tools)
    ...
    log.debug("Initial response %s", response)
    ...
    if should_auto_execute and isinstance(response, ResponsesAPIResponse):
        tool_calls = MCPPayloadUtils._extract_tool_calls_from_response(response=response)
        ...
        if tool_calls:
            ...
            ) = ResponseIdentityManager.extract_mcp_headers_from_request(
            ...
            if tool_results:
                follow_up_input = MCPPayloadUtils._create_follow_up_input(
                ...
                follow_up_call_params = (
                    MCPPayloadUtils._prepare_follow_up_call_params(
                ...
                # Add custom output elements to the final response (for non-streaming)
                    ...
                    final_response = (
                        MCPPayloadUtils._add_mcp_output_elements_to_response(
    ...
    **kwargs,
    ...
    try:
        ...
        if custom_llm_provider is None:
            _, custom_llm_provider, _, _ = get_llm_provider(model=model, api_base=local_vars.get("base_url", None))

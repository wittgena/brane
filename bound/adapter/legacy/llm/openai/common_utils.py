# bound.adapter.legacy.llm.openai.common_utils
## @lineage: anchor.surface.legacy.llm.openai.common_utils
## @lineage: anchor.surface.legacy.openai.common_utils
## @lineage: bound.inter.llms.openai.common_utils
import hashlib
import inspect
import json
import os
...
from bound.broker.transport.mock import MockOpenAITransport
...
class BaseOpenAILLM:
    ...
    def get_cached_openai_client(
        ...
        _cached_client = config.in_memory_llm_clients_cache.get_cache(_cache_key)
    ...
    def set_cached_openai_client(
        ...
        config.in_memory_llm_clients_cache.set_cache(
            key=_cache_key,
            value=openai_client,
    ...
    def _get_async_http_client(
        ...
        if config.aclient_session is not None:
            return config.aclient_session

        if config.network_mock:
    ...
    def _get_sync_http_client() -> Optional[httpx.Client]:
        if config.client_session is not None:
            return config.client_session
        ...
        # Get unified SSL configuration
        ssl_config = get_ssl_configuration()
        return httpx.Client(verify=ssl_config, follow_redirects=True,)
...
def get_openai_credentials(
    ...
    resolved_organization = (
        organization
        or config.organization
        or os.getenv("OPENAI_ORGANIZATION", None)
        or None
    )
    resolved_api_key = (
        api_key or config.api_key or config.openai_key or os.getenv("OPENAI_API_KEY")
    )

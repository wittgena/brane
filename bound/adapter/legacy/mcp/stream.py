# bound.adapter.legacy.mcp.stream
## @lineage: anchor.surface.legacy.mcp.stream
...
from bound.adapter.legacy.llm.openai.types import OutputItemDoneEvent
    ...
    async def _generate_tool_execution_events(self) -> None:
        ...
        try:
            ...
            if self.collected_response is not None:
                tool_calls = MCPPayloadUtils._extract_tool_calls_from_response(self.collected_response)  # type: ignore[arg-type]
            else:
                tool_calls = []
            if not tool_calls:
                return

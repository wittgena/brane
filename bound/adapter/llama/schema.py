# bound.adapter.llama.schema
## @lineage: xphi.adapter.llama.schema
## @lineage: bound.adapter.schema
## @lineage: anchor.adapter.schema
...
from bound.adapter.llama.bridge.pydantic_core import CoreSchema
from bound.adapter.llama.utils import SAMPLE_TEXT, truncate_text

if TYPE_CHECKING:
    from haystack.schema import Document as HaystackDocument
    from semantic_kernel.memory.memory_record import MemoryRecord
    from bound.adapter.llama.base.llms.types import BaseContentBlock
...
class BaseNode(BaseComponent):
    ...
    def get_metadata_content_blocks(
        ...
        from bound.adapter.llama.base.llms.types import TextBlock
...
class ImageNode(TextNode):
    ...
    def get_content_blocks(self, metadata_mode: MetadataMode = MetadataMode.NONE) -> list[BaseContentBlock]:
        from bound.adapter.llama.base.llms.types import ImageBlock
        blocks: list[BaseContentBlock] = []
        blocks.extend(self.get_metadata_content_blocks(metadata_mode))
...
class IndexNode(TextNode):
    ...
    def _serialize_obj(self) -> Any:
        from bound.adapter.llama.storage.docstore.utils import doc_to_json
        try:
            if self.obj is None:
                return None
    ...
    def from_dict(cls, data: Dict[str, Any], **kwargs: Any) -> Self:  # type: ignore
        ...
        if isinstance(obj, str):
            ...
            from bound.adapter.llama.storage.docstore.utils import json_to_doc
            try:

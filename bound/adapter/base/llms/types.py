# bound.adapter.base.llms.types
## @lineage: anchor.adapter.base.llms.types
...
from bound.adapter.constants import DEFAULT_CONTEXT_WINDOW, DEFAULT_NUM_OUTPUTS
from bound.adapter.schema import ImageDocument
from bound.adapter.utils import get_tokenizer, resolve_binary
...
class BaseContentBlock(ABC, BaseModel):
    ...
    def get_template_vars(self) -> list[str]:
        ...
        """
        from bound.adapter.prompts.utils import get_template_vars
    ...
    def format_vars(self, **kwargs: Any) -> "BaseContentBlock":
        ...
        For that to work, the validation on those fields would need to be updated though.
        """
        from bound.adapter.prompts.utils import format_string
...
class TextBlock(BaseContentBlock):
    ...
    async def asplit(
        ...
        from bound.adapter.node_parser import TokenTextSplitter

# channel.cost.calc
## @lineage: channel.bound.cost.calc
## @lineage: gate.bound.cost.calc
import time
...
from channel.cost.tool_call_cost_tracking import StandardBuiltInToolCostTracking
from channel.cost.usage_object_transformation import TranscriptionUsageObjectTransformation
...
from gate.model.types.rerank import RerankBilledUnits, RerankResponse
from gate.model.types.utils import CallTypesLiteral, LiteLLMRealtimeStreamLoggingObject, LlmProvidersSet, StandardBuiltInToolsParams, Usage
from gate.model.types.utils import CallTypes, CostPerToken, EmbeddingResponse, ImageResponse, TextCompletionResponse, TranscriptionResponse
...
def _apply_cost_margin(
    ...
    if custom_llm_provider and custom_llm_provider in litellm.cost_margin_config:
        ...
        log.debug(
            f"No margin config found. Provider: {custom_llm_provider}, Available configs: {list(litellm.cost_margin_config.keys())}"
        )

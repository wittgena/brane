...
DISK_CACHE_DIR = os.environ.get("SPI_CACHEDIR") or os.path.join(Path.home(), ".spi_cache")
DISK_CACHE_LIMIT = int(os.environ.get("SPI_CACHE_LIMIT", 3e10))  # 30 GB default
...
def configure_cache(
    ...
    settings.cache = SPI_CACHE


litellm.telemetry = False
litellm.cache = None  # By default we disable LiteLLM cache and use on-disk cache.


def _get_spi_cache():
    disk_cache_dir = os.environ.get("SPI_CACHEDIR") or os.path.join(Path.home(), ".spi_cache")
    disk_cache_limit = int(os.environ.get("SPI_CACHE_LIMIT", 3e10))

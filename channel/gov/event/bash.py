# channel.gov.event.bash
## @lineage: gov.gateway.event.bash
## @lineage: gov.sandbox.bound.event.bash
## @lineage: gov.sandbox.service.event.bash
## @lineage: gov.sandbox.agent.service.bash
import asyncio
import glob
import json
...
class BashEventService:
    ...
    async def search_bash_events(
        ...
        command_id__eq: ToposId | None = None,
    ...
    async def subscribe_to_events(self, subscriber: Subscriber[BashEventBase]) -> ToposId:

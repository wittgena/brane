# channel.bridge.litellm.logging_worker
## @lineage: bridge.litellm.logging_worker
## @lineage: channel.litellm.logging_worker
## @lineage: channel.bound.litellm.logging_worker
## @lineage: gate.litellm.logging_worker
## @lineage: gate.litellm_core_utils.logging_worker
## @lineage: blm.litellm_core_utils.logging_worker
import asyncio
import contextvars
import logging
...
log = get_emitter("logging.worker")
...
class LoggingWorker:
        ...
        try:
            if self._queue is not None:
                try:
                    ...
                    log.exception(f"LoggingWorker error: {e}")
                finally:
    ...
    async def _worker_loop(self) -> None:
        ...
        try:
            ...
            log.debug("LoggingWorker cancelled during shutdown")
    ...
    def enqueue(self, coroutine: Coroutine) -> None:
        ...
        try:
            ...
            log.exception("LoggingWorker queue is full")
    ...
    def _safe_log(self, level: str, message: str) -> None:
        ...
        if not hasattr(log, "handlers") or not log.handlers:
            return

...
class AnthropicModelInfo(BaseLLMModelInfo):
    ...
    def _supports_model_capability(model: str, key: str) -> bool:
        ...
        try:
            for cand in candidates:
                if cand in model_cost and (model_cost[cand].get(key) is True):
                    return True
        except Exception:
            pass
        return False
    ...
    def get_anthropic_beta_list(
        ...
        """
        from channel.model.types.llms.anthropic import ANTHROPIC_EFFORT_BETA_HEADER
    ...
    def get_anthropic_headers(
        ...
        if tool_search_used or programmatic_tool_calling_used or input_examples_used:
            from channel.model.types.llms.anthropic import ANTHROPIC_TOOL_SEARCH_BETA_HEADER
        ...
        if is_vertex_request is True:
            ...
            if web_search_tool_used:
                from channel.model.types.llms.anthropic import ANTHROPIC_BETA_HEADER_VALUES
    ...
    def get_models(
        ...
        response = HTTPHandler.get(url=f"{api_base}/v1/models", headers=headers)

# xphi.manager.spec.acps.gena.schema
## @lineage: xphi.manager.acp.gena.schema
## @lineage: meta.acp.gena.schema
...
import anchor.surface.acps as target_pkg

log = get_emitter("gena.all")

ACP_ROOT = resolve_path("acp")
BRANE_ROOT = resolve_path("brane")

SCHEMA_JSON = ACP_ROOT / "schema.json"
VERSION_FILE = ACP_ROOT / "VERSION"

# SCHEMA_OUT = Path(target_pkg.__file__).resolve().parent / "schema.py"
SCHEMA_OUT = BRANE_ROOT / "anchor/surface/acps" / "schema.py"
...
def postprocess_generated_schema(output_path: Path) -> list[str]:
    ...
    log.info(f"[SUCCESS] Schema generated at: {output_path}")
